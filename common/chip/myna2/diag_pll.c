...
int get_vco(unsigned int pllbase)
{
    ...
    dbg_printf(PRN_INFO, " current REF DIV is: %d\n", m+1);
    dbg_printf(PRN_INFO, " current DN INT is: %d\n", n_int+1);
    dbg_printf(PRN_INFO, " current DN FRAC is: %d\n", n_frac);

    double nx = n_int +1 + (double)n_frac/(1<<24);
    dbg_printf(PRN_INFO, " current N.X is: %f\n", nx);
...
//        break;
    ...
    case DIAG_APLL1:
        ...
        break;
    default:
        dbg_printf(PRN_RES, "ILLEGAL PLL BASE!!\n");
        pll_base=0xDEADBEEF;
        break;
    }
...
int diag_bypass_pll_global(E_PLL_SRC pll, int bypass)
{
    ...
    {
        BFM_HOST_Bus_Read32(MEMMAP_CHIP_CTRL_REG_BASE + RA_Gbl_siPllClkSwitch, &val);
        val &= ~(1<<0);
        val |= bypass<<0;
        BFM_HOST_Bus_Write32(MEMMAP_CHIP_CTRL_REG_BASE + RA_Gbl_siPllClkSwitch, val);
    }
...
// TB: DOLPHIN ABIPLL combine PLL PD and PLL BYPASS, so BYPASS signal controls
// both.
        ...
        int ssc_amp, int ssc_mode, int calc, APLLCFG_t apll_cfg)
{
	...
	// DOLPHIN TB: no PLL mode selection in ABI PLL
	// change mode
	...
	int ssc_mode, unsigned int calc, APLLCFG_t pll_cfg)
{
    ...
    // APLL1=3
    if(apll_index<2){
	BFM_HOST_Bus_Read32(ctrl0base, &vpllctrl.u32);
    }else if(apll_index<4){
	BFM_HOST_Bus_Read32(ctrl0base, &apllctrl.u32);
    }
    ...
    BFM_HOST_Bus_Write32(cfg1base, mclkctrl.u32);

    // using APLL for Audio module (CTRL0)
    T32AVPLL_WRAP_CTRL0 mclkctrl0;
    BFM_HOST_Bus_Read32(ctrl0base, &mclkctrl0.u32);
    mclkctrl0.u32 = 0;
    BFM_HOST_Bus_Write32(ctrl0base, mclkctrl0.u32);
#endif
    return ret;
}
