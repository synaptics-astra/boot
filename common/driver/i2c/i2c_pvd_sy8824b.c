...
#ifndef MINIMAL_VOL_STEP_SY8824B
#define MINIMAL_VOL_STEP_SY8824B 12500
#endif
...
static int sy8824b_set_vol(int master_id, int slaveAddr, int volt)
{
	int target = volt;
	...
	if (ret == 0) {
		...
		if (data_reg0 != data) {
			dbg_printf(PRN_ERR, "PMIC sy8824b: read:0x%02x != write:0x%02x\n", data, data_reg0);
			return -1;
		}
		mdelay(1);
	}
...
static int sy8824b_get_vol(int master_id, int slaveAddr)
{
        ...
        ret |= pmic_data2volt(&volt, (int)read);
        if(ret) {
...
{
	int volt = sy8824b_get_vol(VOUT_CPU_ID, SLAVE_ADDR);
...
{
	int volt = sy8824b_get_vol(VOUT_CORE_ID, SLAVE_ADDR);
...
static int i2c_set_volt(int master_id, int from, int to)
{
	...
	while (volt != to){
		...
		if(0 != sy8824b_set_vol(master_id, SLAVE_ADDR, volt))
			return 1;
		//udelay(50); 	//FIXME: in sy8824b_set_vol, 1ms delayed
	}
