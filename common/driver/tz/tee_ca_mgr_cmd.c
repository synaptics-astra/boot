	...
	uint32_t*		taskId)
{
	...
	if ((status & TZ_COMM_RSP_DONE) && !ret.res.result) {
		*taskId = p->taskId;
		dbg_printf(PRN_INFO,"TAMgr_CreateInstance:tz comm rsp done! %x, %x, %x\n", status, ret.res.result, p->taskId);
	} else {
		dbg_printf(PRN_ERR,"TAMgr_CreateInstance:tz comm rsp error! %x, %x\n", status, ret.res.result);
	}
	...
	uint32_t*		returnOrigin)
{
	...
	if ((status & TZ_COMM_RSP_DONE) && !ret.res.result) {
		dbg_printf(PRN_INFO,"TAMgr_DestroyInstance:tz comm rsp done! %x, %x\n", status, ret.res.result);
	} else {
		dbg_printf(PRN_ERR,"TAMgr_DestroyInstance:tz comm rsp error! %x, %x\n", status, ret.res.result);
	}
