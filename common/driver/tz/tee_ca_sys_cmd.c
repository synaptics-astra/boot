	...
	uint32_t*		sessionId)
{
	...
	if (!comm || !taskId) {
		dbg_printf(PRN_ERR, "invalid comm(0x%08x) or taskId(0x%08x)\n",
	...
	case TEEC_LOGIN_GROUP_APPLICATION:
		if(connectionData == NULL) {
			dbg_printf(PRN_ERR, "connection method requires valid group\n");
			result = TEEC_ERROR_BAD_PARAMETERS;
			goto out;
		} else {
		...
		break;
	default:
		dbg_printf(PRN_ERR, "invalid connectionMethod (0x%08x)\n", connectionMethod);
		result = TEEC_ERROR_BAD_PARAMETERS;
		goto out;
	}
	...
	dbg_printf(PRN_INFO, "invoke command 0x%08x\n", cmd->cmd_id);
	...
	if ((status & TZ_COMM_RSP_DONE) && !ret.res.result) {
		...
		*sessionId = p->sessionId;
		dbg_printf(PRN_INFO, "TASysCmd_OpenSession:tz comm rsp done! %x, %x, %x\n", status, ret.res.result, p->sessionId);
	} else {
		result = TEEC_ERROR_GENERIC;
		dbg_printf(PRN_ERR, "TASysCmd_OpenSession:tz comm rsp error! %x, %x\n", status, ret.res.result);
	}
	...
	bool*			instanceDead)
{
	...
	if ((status & TZ_COMM_RSP_DONE) && !ret.res.result) {
		...
		*instanceDead = cmd->params[0].value.a;
		dbg_printf(PRN_INFO, "TASysCmd_CloseSession:tz comm rsp done! %x, %x\n", status, ret.res.result);
	} else {
		res = TEEC_ERROR_GENERIC;
		dbg_printf(PRN_ERR, "TASysCmd_CloseSession:tz comm rsp error! %x, %x\n", status, ret.res.result);
	}
