				...
				uint32_t cmdId, TEEC_Operation *operation)
{
	...
	if (!cmd) {
		dbg_printf(PRN_ERR, "illigle arguments\n");
...
out:
	//dbg_printf(PRN_NOTICE, "invoke command 0x%08x\n", cmd->cmd_id);
	return TEEC_SUCCESS;
}
...
TEEC_Result register_ta(void * pta, size_t size)
{
	...
	comm = (struct tee_comm *)(uintptr_t)mmgr_alloc_by_type(MEM_ION_CACHEABLE, sizeof(struct tee_comm));

	if(!comm ) {
		dbg_printf(PRN_ERR, "register_ta: no enough memory!\n");
		goto done;
	}
	...
	if ((status & TZ_COMM_RSP_DONE) && !ret.res.result) {
		result = TEEC_SUCCESS;
		dbg_printf(PRN_INFO,"register_ta:tz comm rsp done! %x, %x\n", status, ret.res.result);
	} else {
		result = TEEC_ERROR_GENERIC;
		dbg_printf(PRN_ERR,"register_ta:tz comm rsp error! %x, %x\n", status, ret.res.result);
	}
		...
		uint32_t*		returnOrigin)
{
	...
	if((session == NULL) || (destination == NULL)) {
		dbg_printf(PRN_ERR, "Illegal argument\n");
		return TEEC_ERROR_BAD_PARAMETERS;
	}

	memset(session, 0, sizeof(*session));

	session->comm = (struct tee_comm *)(uintptr_t)mmgr_alloc_by_type(MEM_ION_CACHEABLE, sizeof(struct tee_comm));

	session->comm->pa = (uint32_t)((unsigned long)session->comm);
        session->comm->va = (uint32_t)((unsigned long)session->comm);

	if (!session->comm){
		dbg_printf(PRN_ERR, "fail to get comm channel\n");
	...
	if (res != TEEC_SUCCESS) {
		dbg_printf(PRN_ERR, "fail to create instance, result=0x%08x\n", res);
		goto fail_out;
	}
	...
	if (res != TEEC_SUCCESS) {
		dbg_printf(PRN_ERR, "fail to open session, result=0x%08x\n", res);
		goto fail_out;
	}
		...
		TEEC_Session*		session)
{
	...
	if(!session || !session->comm ||
		...
		dbg_printf(PRN_ERR, "invalid session handle\n");
		return;
	}

	if(session->operationCount > 0) {
		dbg_printf(PRN_ERR, "%d Pending operations\n",
	...
	if (res != TEEC_SUCCESS) {
		dbg_printf(PRN_ERR, "fail to close session, result=0x%08x\n", res);
		return;
	}
		...
		TEEC_Operation*	operation)
{
	...
	//check operation count to do mutex protection
	if (session->operationCount > 0)
	...
	if ((status & TZ_COMM_RSP_DONE) && !ret.res.result) {
		res = TEEC_SUCCESS;
		//dbg_printf(PRN_NOTICE,"TEEC_InvokeCommand:tz comm rsp done! %x, %x\n", status, ret.res.result);
		TEEC_CallCommandToOperaction(operation, &session->comm->call_param);
	} else {
		res = TEEC_ERROR_GENERIC;
		dbg_printf(PRN_ERR,"TEEC_InvokeCommand)(%d/%x):tz comm rsp error! %x, %x\n", commandID, commandID, status, ret.res.result);
	}
